...
def preprocess_dataset(dataset, tokenizer, max_length=384):
    ...
    def format_example(example):
        ...
        try:
            ...
            # Tokenize input with language tags
            inputs = tokenizer(
                input_text,
            ...
            # Tokenize target without language tags, using text_target for decoder
            targets = tokenizer(
                text_target=target_text,  # Use text_target for target language
                max_length=max_length,
                truncation=True,
                padding="max_length",
                return_tensors="pt",
    ...
    if len(processed_dataset) == 0:
        logger.warning("No valid examples remaining. Saving error examples for debugging.")
        error_dataset = dataset.filter(lambda x, idx: format_example(x) is None, with_indices=True)
        error_dataset.save_to_disk("debug_error_dataset")
        logger.info("Saved error examples to 'debug_error_dataset'")
        raise ValueError("No valid examples remaining after preprocessing! Please check 'debug_error_dataset'.")
